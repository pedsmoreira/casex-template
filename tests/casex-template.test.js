...
describe("casexTemplate", () => {
	...
	it("pluralizes and singularizes composed _name_", () => {
		...
		const transformedText =
			"Create new user folders on the current user folder";

		expect(casexTemplate({ name: "user_folder", text })).toEqual(
			transformedText,
		);
	...
	it("replaces __name__ with numbers accordingly", () => {
		...
		expect(casexTemplate({ name: "john1-doe2", text })).toEqual(
			transformedText,
		);
	});
});
