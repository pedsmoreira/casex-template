import { replacePatterns } from '../src';
...
describe('#replacePatterns', () => {
  it('doesnt break when there are no matches', () => {
    ...
    expect(replacePatterns(text, 'john-doe')).toEqual(transformedText);
  });
  ...
  it('returns the text when no name is provided', () => {
    ...
    expect(replacePatterns(text, null)).toEqual(transformedText);
    expect(replacePatterns(text, '')).toEqual(transformedText);
  });
  ...
  it('does not pluralize __name__', () => {
    ...
    expect(replacePatterns(text, 'person')).toEqual(transformedText);
  });
  ...
  it('does not singularize __name__', () => {
    ...
    expect(replacePatterns(text, 'people')).toEqual(transformedText);
  });
  ...
  it('pluralizes and singularizes regular _name_', () => {
    ...
    expect(replacePatterns(text, 'duck')).toEqual(transformedText);
    expect(replacePatterns(text, 'ducks')).toEqual(transformedText);
  });
  ...
  it('pluralizes and singularizes composed _name_', () => {
    ...
    expect(replacePatterns(text, 'user_folder')).toEqual(transformedText);
    expect(replacePatterns(text, 'user_folders')).toEqual(transformedText);
  });
  ...
  it('understands when _na_me_ is part of the replaced content', () => {
    ...
    expect(replacePatterns(text, 'a_na_me_b')).toEqual(transformedText);
  });
  ...
  it('replaces __name__ with numbers accordingly', () => {
    ...
    expect(replacePatterns(text, 'john1-doe2')).toEqual(transformedText);
  });
});
